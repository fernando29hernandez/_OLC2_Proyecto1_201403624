...
public class POTENCIA extends Expresion {
    ...
    public POTENCIA(Expresion op1, Expresion op2, String operando, int fila, int columna)
    {
        this.op1 = op1;
        this.op2 = op2;
    ...
    public Object Ejecutar(Entorno tablasimbolos) {
        ...
        PRIMITIVO resultado = new PRIMITIVO(new Tipo(Tipo.TipoSimbolo.entero), 0, fila, columna); //Por defecto voy a retornar un booleano
        ...
        if (resultado1.tipo.tipo.equals(Tipo.TipoSimbolo.entero))
        {
            if (resultado2.tipo.tipo.equals(Tipo.TipoSimbolo.entero))
            {
                ...
                resultado.valor = (int)Math.pow(valor_ , valor_2);
...
//                Double valor_ = Double.valueOf(resultado1.valor.toString());
//                Double valor_2 = Double.valueOf(resultado2.valor.toString());
//                resultado.valor = Math.pow(valor_, valor_2);
//                resultado.tipo.tipo = Tipo.TipoSimbolo.doble;
//                return resultado;
//            }
            else if(resultado2.tipo.tipo.equals(Tipo.TipoSimbolo.caracter))
            {
                Double valor = Double.valueOf(resultado1.valor.toString());
                int valor_ = (int) Math.pow(valor.intValue(),resultado2.valor.toString().charAt(0));
                resultado.valor = valor_;
                resultado.tipo.tipo = Tipo.TipoSimbolo.entero;
                return resultado;
        ...
        else if (resultado1.tipo.tipo.equals(Tipo.TipoSimbolo.caracter))
        {
            switch (resultado2.tipo.tipo) {
                case entero:
                {
                    ...
                    resultado.valor = Math.pow(valor__,valor__2);
                    resultado.tipo.tipo = Tipo.TipoSimbolo.entero;
                    return resultado;
                }
//                case doble:
...
//                    int valor__ = valor_.charAt(0);
//                    Double valor_2 = Double.valueOf(resultado2.valor.toString());
//                    resultado.valor = Math.pow(valor__,valor_2);
//                    resultado.tipo.tipo = Tipo.TipoSimbolo.doble;
//                    return resultado;
//                }
                case caracter:
                    resultado.valor = (int)Math.pow(resultado1.valor.toString().charAt(0),resultado2.valor.toString().charAt(0));
                    resultado.tipo.tipo = Tipo.TipoSimbolo.entero;
                    return resultado;
                default:
                    break;
