 ...
 * @author Fernando
 */
public class ESCRIBIR extends Instruccion {

    LinkedList<Expresion> valores;
    int fila;
    int columna;

    public ESCRIBIR(LinkedList<Expresion> valores, int fila, int columna) {
    ...
    public Object Ejecutar(Entorno tablasimbolos) {
        if (valores.size() == 0) {
            Errores.agregar_error("No hay parametros para la funcion escribir", fila, columna, "Semantico");
            return null;
        }
        if (Display.bw == null) {
            Errores.agregar_error("No se puede usar la funcion escribir dado a que no se ha abierto ningun arichivo", fila, columna, "Semantico");
            return null;
        }
            ...
            if (sim.Tipo.tipo == Tipo.TipoSimbolo.objeto
                ...
                try {
                    ...
                    if (indice_entero != -1 || indice_decimal != -1 && indice_caracter != -1 || indice_booleano != -1 || indice_char_array != -1) {
                        Errores.agregar_error("Escritura de consola erronea debido a falta de parametros", fila, columna, "Semantico");

                        return null;
                    }
                    Display.bw.write(impresion);
